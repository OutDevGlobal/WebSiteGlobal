...
export const Star = () => {
  ...
  useEffect(() => {
    ...
    if (!isLoaded){
      ...
      loaderObj.load('../gltf/forweb.glb', (gltf) => {
        meshObj = gltf.scene.getObjectByName('test01')

        const materialChange = new THREE.MeshPhysicalMaterial({
          color: 'rgb(33,21,135)', wireframe: true, roughness: 0.2,
          transmission: 0.8, metalness: 0.3, emissive: new THREE.Color('rgb(33,21,135)'),
